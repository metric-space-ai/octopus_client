...
function _FileMarkDownContent(props: {content: string}) {
  return (
    <ReactMarkdown
...
export const FileMarkdownContent = React.memo(_FileMarkDownContent);
...
export function FileMarkdown(
  ...
  return (
    ...
    >
      ...
      {inView.current &&
        (props.loading ? <EllipsisHorizontalIcon className='w-6 h-6' /> : <FileMarkdownContent content={props.content} />)}
    </div>
  );
}
